...
export default async function process_sonos_command(received_text) {
  ...
  if (service_type != 'command') {
    ...
    let res;
    res = await fetch(sonos_http_api + '/' + sonos_room + '/repeat/off');
    if (!res.ok)
      throw new Error(
        `Unexpected response while turning repeat off: ${res.status}`
      );
    await new Promise((resolve) => setTimeout(resolve, 200));
    res = await fetch(sonos_http_api + '/' + sonos_room + '/shuffle/off');
    if (!res.ok)
      throw new Error(
        `Unexpected response while turning shuffle off: ${res.status}`
      );
    res = await fetch(sonos_http_api + '/' + sonos_room + '/crossfade/off');
    if (!res.ok)
      throw new Error(
        `Unexpected response while turning crossfade off: ${res.status}`
      );
    res = await fetch(sonos_http_api + '/' + sonos_room + '/clearqueue');
    if (!res.ok)
      throw new Error(
