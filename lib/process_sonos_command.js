...
export default async function process_sonos_command(received_text) {
  ...
  if (received_text_lower.startsWith('apple:')) {
    ...
    const sonos_room = received_text.slice(5);
    config.set('sonos_room', sonos_room);
    logger.info(`Sonos room changed to ${sonos_room}`);
    return;
  }
  ...
  logger.info("Detected '%s' service request", service_type);

  const urltoget = get_sonos_url(sonos_instruction, service_type);

  // Reset playback options (depending on configuration)
  if (service_type !== 'command') {
    await reset_playback_options();
  }

  // Perform the requested action on the sonos API
  logger.info('Fetching URL via HTTP api: %s', urltoget);
  ...
  logger.info('Sonos API reports: ', await res.json());
